...
const AddProperties = () => {
  ...
  // Sa AddProperties component, ilagay ito sa useEffect:
  useEffect(() => {
    // Reset payment status on page load for testing
    const resetPaymentForTesting = true; // Set to false kapag production na
    if (resetPaymentForTesting) {
      localStorage.removeItem('pano_payment_status');
      localStorage.removeItem('pano_payment_session');
      setPaidForPano(false);
      console.log('🔄 Payment status reset for testing');
    }
  }, []);
  ...
  const [paidForPano, setPaidForPano] = useState(() => {
    try {
      return localStorage.getItem('pano_payment_status') === 'true';
    } catch (e) { return false; }
  });
  const [paymentSessionId, setPaymentSessionId] = useState(() => {
    try { return localStorage.getItem('pano_payment_session') || null; } catch (e) { return null; }
  });

  // Refs to file inputs so we can reset them programmatically
  const panoramaInputRef = useRef(null);
  const imageInputRef = useRef(null);
  const videoInputRef = useRef(null);

  // FIXED: Improved panorama handling with proper payment logic
  const handlePanoramaChange = (e) => {
    const files = Array.from(e.target.files || []);
    if (!files.length) return;

    // ✅ FIX: Use current count + ALL files (not filtered) for payment check
    const currentCount = panoramas.length;
    const totalCountIfAdded = currentCount + files.length;

    // ✅ FIX: Use the correct parameter name that matches the helper
    const helperResult = handlePanoUploadLogic({
      currentCount: panoramas.length,  // ✅ Dapat 'currentCount' hindi 'panoCount'
      paidForPano: paidForPano
    });
    ...
    if (!helperResult.allowed) {
      if (helperResult.message === 'Show payment modal') {
        console.log('🎯 Showing payment modal for', files.length, 'files');
        // Put files into pending state and open payment modal
        setPendingPanoramaFiles(files);
        // generate a temporary session id and persist it if none exists
        if (!paymentSessionId) {
        ...
        // ✅ CRITICAL: Clear file input and return immediately
    ...
    // ✅ Only process files if payment check passed
    ...
    // Generate unique ids and create preview URLs
    const genId = () => (typeof crypto !== 'undefined' && crypto.randomUUID) ? crypto.randomUUID() : `${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 8)}`;

    const existingKeys = new Set(panoramas.map(p => p.file.name + '|' + p.file.size));
  ...
  // Payment modal handlers: process pending files after successful payment
  const handlePanoPaymentSuccess = () => {
    ...
    // Convert pending files into panorama items and add to state
    const genId = () => (typeof crypto !== 'undefined' && crypto.randomUUID) ? crypto.randomUUID() : `${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 8)}`;
    const existingKeys = new Set(panoramas.map(p => p.file.name + '|' + p.file.size));
  ...
  const handlePanoPaymentCancel = () => {
    // Clear pending files and close modal without adding previews
    setPendingPanoramaFiles([]);
    setShowPanoPayment(false);
    if (panoramaInputRef.current) panoramaInputRef.current.value = '';
  };

  const removePanorama = (id) => {
    setPanoramas(prev => {
    ...
    // also reset file input value so selecting same file works later
    if (panoramaInputRef.current) panoramaInputRef.current.value = '';
  };
  ...
  // FIX: Reset form when entering the page
  useEffect(() => {
    // Reset form when component mounts (fresh visit)
    if (location.pathname === '/add-properties' && !location.state?.preserveForm) {
      resetFormState();
    }
  ...
  const [propertyData, setPropertyData] = useState(initialFormState);

  // FIX: Form reset function
  const resetFormState = () => {
    setPropertyData(initialFormState);
    setImagePreviews([]);
    setPanoramas([]);
  ...
  const [, setPriceFocused] = useState(false);
  const [priceError, setPriceError] = useState('');
  const [imagePreviews, setImagePreviews] = useState([]); // stored as { id, url }
  ...
  const geocodeAddress = async (address, barangay) => {
    ...
    try {
      ...
      if (err.name === 'AbortError') {
        ...
        } catch (err) { console.warn('Nominatim fallback after abort failed', err); }
        ...
        } catch (err) { console.warn('Nominatim fallback after error failed', err); }
  ...
  function MapCenterSync({ lat, lng }) {
    ...
    useEffect(() => {
      ...
      } catch (err) { console.warn('Map center sync failed', err); }
    }, [lat, lng, map]);
    return null;
  }

  MapCenterSync.propTypes = {
  ...
  // FIX: Enhanced submission locking
  ...
  const handleImageChange = (e) => {
    ...
    const genId = () => (typeof crypto !== 'undefined' && crypto.randomUUID) ? crypto.randomUUID() : `${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 8)}`;
    const existingKeys = new Set(propertyData.images.map(i => i.file.name + '|' + i.file.size));
    const newItems = usable.filter(f => !existingKeys.has(f.name + '|' + f.size)).map(f => ({ id: genId(), file: f }));
    if (!newItems.length) { if (e.target) e.target.value = ''; return; }
    setPropertyData(p => ({ ...p, images: [...p.images, ...newItems] }));
    setImagePreviews(p => [...p, ...newItems.map(item => ({ id: item.id, url: URL.createObjectURL(item.file) }))]);
    // reset input so same file can be selected again after removal
    if (e.target) e.target.value = '';
  };

  const removeImage = (id) => {
    setPropertyData(p => ({ ...p, images: p.images.filter(img => img.id !== id) }));
  ...
  useEffect(() => {
    return () => {
      imagePreviews.forEach(p => p && p.url && URL.revokeObjectURL(p.url));
      if (videoPreview && videoPreview.startsWith && videoPreview.startsWith('blob:')) URL.revokeObjectURL(videoPreview);
      panoramas.forEach(p => p && p.previewUrl && URL.revokeObjectURL(p.previewUrl));
    };
  }, [imagePreviews, videoPreview, panoramas]);
  ...
  const handleVideoChange = (e) => {
    ...
    if (propertyData.video && videoPreview && videoPreview.startsWith && videoPreview.startsWith('blob:')) URL.revokeObjectURL(videoPreview);
  ...
  // FIX: Enhanced submission handler with duplicate prevention
  const handleSubmit = async (e) => {
    e.preventDefault();

    // STRONGER SUBMISSION LOCK
    ...
    // Disable all form controls to prevent user interaction while submitting
    if (formRef.current) {
      const inputs = formRef.current.querySelectorAll('input, button, select, textarea');
    ...
    try {
      ...
      // Small debounce to avoid rapid successive submissions
      await new Promise(resolve => setTimeout(resolve, 500));

      const formData = new FormData();

      // ✅ CRITICAL MISSING CODE: FORCE PAYMENT STATUS FOR NEW PROPERTIES
      if (panoramas.length > 0) {
        formData.append('paidForPano', paidForPano.toString());
        if (paymentSessionId) {
          formData.append('paymentSessionId', paymentSessionId);
        }
        console.log('💰 Sending payment status to backend:', { 
          paidForPano, 
          paymentSessionId,
          panoramasCount: panoramas.length 
        });
      }
      ...
      // Navigate away with replace to prevent back button issues
      ...
      // Small delay before allowing next submission to avoid rapid retries
      setTimeout(() => {
  ...
  return (
    <div className="dashboard-container landlord-dashboard">
      ...
      <div className="landlord-main add-property-main">
        <form ref={formRef} onSubmit={handleSubmit} className="ll-card add-property-form" noValidate>
            ...
            <div className="ll-stack">
              <div className="images-section" style={{ marginTop: '0' }}>
                ...
                <div className="current-images-grid">
                  {imagePreviews.length ? imagePreviews.map((p, i) => (
                    <div key={p.id} className="image-chip">
                      <img src={p.url} alt={`Property ${i}`} />
                      <button type="button" aria-label="Remove image" onClick={() => removeImage(p.id)}>&times;</button>
                    </div>
                  )) : <div className="placeholder">No images</div>}
                </div>
                <div className="new-upload-block">
                  <label className="file-drop-modern">
                    <input ref={imageInputRef} type="file" multiple accept="image/*" onChange={handleImageChange} />
              ...
              <div className="panorama-section" style={{ marginTop: '32px' }}>
                ...
                <label className="file-drop-modern">
                  <input ref={panoramaInputRef} id="panorama-input" type="file" accept="image/*" multiple style={{ display: 'none' }} onChange={handlePanoramaChange} />
                  <span onClick={() => panoramaInputRef.current && panoramaInputRef.current.click()}>Add 360° Panoramic Images</span>
                </label>
                {panoramas.length > 0 && (
              ...
              <div className="video-section" style={{ marginTop: '32px' }}>
                ...
                {!videoPreview && !propertyData.video && (
                  <label className="file-drop-modern">
                    <input ref={videoInputRef} type="file" accept="video/mp4,video/webm,video/ogg" onChange={handleVideoChange} />
        ...
        {showPanoPayment && (
          <PanoPaymentModal
            ...
            onPaymentError={() => {
              // clear pending files and session on error/cancel
              handlePanoPaymentCancel();
              try { localStorage.removeItem('pano_payment_session'); } catch (e) { }
            }}
            propertyId={null}
          />
        )}
